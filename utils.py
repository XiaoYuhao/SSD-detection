...
def calculate_mAP(det_boxes, det_labels, det_scores, true_boxes, true_labels, true_difficulties):
    ...
    """
    print(len(det_boxes), len(det_labels), len(det_scores), len(true_boxes), len(
        true_labels), len(
        true_difficulties))
...
import time
def detect_objects(priors_cxcy, predicted_locs, predicted_scores, min_score, max_overlap, top_k, n_classes):
        """
        ...
        for i in range(batch_size):
            ...
            for c in range(1, n_classes):
                ...
                now = time.time()
                #print("sort time:%f" %(now - tt))
                tt = now

                # Find the overlap between predicted boxes
                overlap = find_jaccard_overlap(class_decoded_locs, class_decoded_locs)  # (n_qualified, n_min_score)
                now = time.time()
                #print("overlap time:%f" %(now - tt))
                tt = now
...
def bbox2result(bboxes, labels, num_classes):
    if bboxes.shape[0] == 0:
        return [np.zeros((0, 5), dtype=np.float32) for i in range(1,num_classes)]
    else:
        bboxes = bboxes.cpu().detach().numpy()
        labels = labels.cpu().detach().numpy()
        return [bboxes[labels == i, :] for i in range(1,num_classes)]       #从1开始，0是背景
...
def bboxes_iou(bboxes1, bboxes2, eps=1e-6):
    ''' 
    Params:
    ...
    if bboxes1.shape[0] == 0 or bboxes2.shape[0] == 0:
        ious = np.zeros((bboxes1.shape[0], bboxes2.shape[0]), dtype=np.float32)
        return ious
    x1y1 = np.maximum(bboxes1[:,None,:2], bboxes2[:,:2])
    x2y2 = np.minimum(bboxes1[:,None,2:], bboxes2[:,2:])
    
    wh = np.maximum(x2y2 - x1y1, 0)
    overlap = wh[:,:,0] * wh[:,:,1]
    areas1 = (bboxes1[:,2] - bboxes1[:,0]) * (bboxes1[:,3] - bboxes1[:,1])
    areas2 = (bboxes2[:,2] - bboxes2[:,0]) * (bboxes2[:,3] - bboxes2[:,1])
    union = np.maximum(areas1[:, None] + areas2 - overlap, eps)
